...
- **Two signatures, on the fast path.** A withdrawal needs the owner's signature, checked by the program against that record, so a non-owner can never withdraw. It also needs the settler's co-signature, which is the server saying nothing is in flight. Tokens can only go to the owner's own token account, and the server pays the fee.
- **One signature, on the path that does not need us.** `request_exit` and `claim_exit` are owner-signed and take no settler at all. The window between them replaces what the co-signature attested: instead of the server promising nothing is in flight, the chain gives it thirty minutes to settle whatever is. The owner pays their own fees there, and the destination is still their own token account. See [non-custodial-exit.md](non-custodial-exit.md).
...
3. **Closed, on devnet.** This said the vaults were custodial in practice, because a server that refused or disappeared stranded the money. `request_exit` and `claim_exit` are built and deployed, need no signature but the owner's, and are driven by a page that has no dependency on this server at all ([`web/public/exit/`](../web/public/exit/)). What remains is not the mechanism but its reach: it has not been exercised end to end by a real wallet on devnet, and the page is served from our own hosting, which is the one place it should not only live.
...
12. **One match's worth of exposure on a non-custodial exit, once that ships.** Accepted deliberately, and bounded. A claim can only be short if the server started a match in the moment between `request_exit` landing on chain and the server ingesting it, and then stayed down for the whole thirty-minute window — a match is otherwise settled long before the claim, because the agent is frozen from ingest onward. The most a single unsettled match can move is `max_settlement`, band C's worst match, 60 chips, so that is the ceiling on what a claim can take that it did not own.

    A reserve would close it: hold `max_settlement` back on every unilateral claim. That was considered and rejected — it strands 60 chips on every honest exit to insure against a compound failure that needs the server to die inside a one-pass window and stay dead for half an hour. The exposure is smaller than the cure. It is listed here so that it is a known number rather than a surprise, and it is the figure to revisit if the window is ever shortened.

13. **Renting doesn't prove consent.** An agent's id binds it to its owner, so nobody can record a different one, but the server still creates the agent without the owner signing anything. It could create an agent "owned" by a wallet that never asked for one, which gives that wallet tokens and takes nothing. A transaction signed by the owner at rent time would prove consent; it would cost a signing step and make the agent wait to play.
