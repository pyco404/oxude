...
// The **seed-funded** program's setup: the one frozen at EKJHJ8js... It is
// already deployed and initialised on devnet, so nothing here needs running
// again in the ordinary course. It is kept because it is the only way to touch
// that program's config - rotating its settler is the response to a stolen key,
// and the frozen program still holds the vaults of every agent rented before
// the deposit flow.
//
// For the program the repo now builds, see scripts/chain-setup.ts.
//
//   CHAIN_RPC_URL=https://api.devnet.solana.com npx tsx scripts/seed-chain-setup.ts
//
