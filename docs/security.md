...
3. **Still partly custodial.** Owners can withdraw, but only with the server's co-signature, so the server can refuse or delay a withdrawal. (Depositing back into a vault is built: `deposit`, on the v2 program.) [non-custodial-exit.md](non-custodial-exit.md) is the plan for closing this — an owner-signed `request_exit`, a thirty-minute window in which the server settles what it owes, and an owner-signed `claim_exit` that needs no co-signature. Not built.
