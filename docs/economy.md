...
11. **Who funds the house roster, and who carries its losses.** With no minting, house agents hold real tokens like anyone else, and house matches are zero-sum and settle on chain — so on mainnet the platform is staking its own money against players and can lose it. On devnet this costs nothing and the treasury funds the roster. On mainnet there are three answers and no decision: house agents play unstaked exhibitions, which changes what a house match is; or the roster is funded as a deliberate marketing cost with a stated cap and a published balance; or house matches stop settling on chain. The first is the cleanest and the furthest from what is built, since [Autoplay](#autoplay) falls back to house agents whenever a band is thin, and thin bands are exactly what an early mainnet has. Note also that house matches already earn no ranking and no prizes ([Decided](#decided)), so what is at stake here is only the money, not the ladder.
